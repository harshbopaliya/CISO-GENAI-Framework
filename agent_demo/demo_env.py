...
class GridWorldEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        # FIX: Changed self.np_random.randint to self.np_random.integers
        self.agent_pos = {f"agent_{i}": self.np_random.integers(0, self.grid_size, size=2, dtype=np.int32)
                          for i in range(self.num_agents)}
        self.current_step = 0
        print(f"\n--- Environment Reset --- Initial Agent Positions: {self.agent_pos}")
        return self._get_obs(), {} # Return observations and an info dictionary
    ...
    def step(self, actions):
        ...
        # Print current agent positions for monitoring
        print(f"  Step {self.current_step} - Agent Positions: {self.agent_pos} - Rewards: {rewards}")
